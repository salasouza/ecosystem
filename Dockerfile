...
RUN wget -c --header "Cookie: oraclelicense=accept-securebackup-cookie" https://download.oracle.com/java/22/latest/jdk-22_linux-x64_bin.tar.gz -P /usr/local/.
RUN tar -xzvf /usr/local/jdk-22_linux-x64_bin.tar.gz -C /usr/local/ 
RUN ln -s /usr/local/jdk_22/ /usr/local/java
